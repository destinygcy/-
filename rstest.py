...
'''pipe = Pipeline.Pipeline(None)  # 定义流程pipeline
...
for i in range(count):
    ...
    if videoProfile.format() == StreamProfile.PY_FORMAT_YUYV or videoProfile.format() == StreamProfile.PY_FORMAT_Y16:
        ...
        break
config = Pipeline.Config()  # 定义配置config
...
def get_aligned_images():
    frames = dev.waitForFrames(100)  # 等待获取图像帧
    aligned_frames = dev.getFrame(frames)  # 获取对齐帧
    aligned_depth_frame = aligned_frames.getDepthFrame()  # 获取对齐帧中的depth帧
    color_frame = aligned_frames.getColorFrame()  # 获取对齐帧中的color帧


    ############### 相机参数的获取 #######################
    intr = color_frame.getCameraIntrinsic(2)  # 获取相机内参
    depth_intrin = aligned_depth_frame.getCameraIntrinsic(3)  # 获取深度参数（像素坐标系转相机坐标系会用到）
        ...
        while True:
            ...
            if xyxy_list:
                for i in range(len(xyxy_list)):
                    ...
                    camera_xyz = dev.rs2_deproject_pixel_to_point(
